...
const trainingSchema = z.object({
  ...
  topics: z.array(z.object({
    id: z.string().optional(), // Include ID if topics already exist
...
export function EditTrainingModal({ training, onClose, onUpdate }: EditTrainingModalProps) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await onUpdate(training._id, data); // Call onUpdate with training ID and form data
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center overflow-y-auto">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl my-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">Edit Training Module</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => append({ id: undefined, title: '', videoId: '' })}
            ...
            {fields.map((field, index) => (
              <div key={field.id || index} className="bg-gray-50 p-4 rounded-lg space-y-4">
                <div className="flex items-center justify-between">
